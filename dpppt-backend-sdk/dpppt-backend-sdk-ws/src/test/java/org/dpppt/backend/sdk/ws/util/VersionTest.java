  ...
  public void testVersionFromString() throws Exception {
    ...
    for (int i = 0; i < cases.size(); i++) {
      ...
      for (int j = 0; j < i; j++) {
        ...
        assertFalse(currentVersion.isSameVersionAs(olderVersion));
        assertFalse(currentVersion.isSmallerVersionThan(olderVersion));

        assertFalse(olderVersion.isLargerVersionThan(currentVersion));
        assertFalse(olderVersion.isSameVersionAs(currentVersion));
        assertTrue(olderVersion.isSmallerVersionThan(currentVersion));
      }
    }
