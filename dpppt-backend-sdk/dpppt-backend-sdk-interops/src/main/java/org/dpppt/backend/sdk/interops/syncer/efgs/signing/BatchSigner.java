...
public class BatchSigner {
  ...
  private final String signAlgorithmName;

  public BatchSigner(CryptoProvider cryptoProvider, String signAlgorithmName) {
  ...
  private byte[] createBytesToSign(final DiagnosisKeyBatch batch) {
    ...
    sortBatchByKeyData(batch).stream()
        ...
        .sorted(Comparator.nullsLast(Comparator.comparing(this::bytesToBase64String)))
  ...
  private byte[] createBytesForKey(final DiagnosisKey diagnosisKey) {
    ...
    writeSeparatorInArray(batchBytes);
    writeVisitedCountriesInByteArray(diagnosisKey.getVisitedCountriesList(), batchBytes);
    writeSeparatorInArray(batchBytes);
  ...
  private void writeStringInByteArray(
      final String batchString, final ByteArrayOutputStream byteArray) {
    byteArray.writeBytes(batchString.getBytes(StandardCharsets.US_ASCII));
  }

  private void writeB64StringInByteArray(
      final String batchString, final ByteArrayOutputStream byteArray) {
  ...
  private void writeVisitedCountriesInByteArray(
      final ProtocolStringList countries, final ByteArrayOutputStream byteArray) {
  ...
  private SignerInfoGenerator createSignerInfo(X509Certificate cert)
    ...
    return new JcaSignerInfoGeneratorBuilder(createDigestBuilder())
        .build(createContentSigner(), cert);
  }
  ...
  private ContentSigner createContentSigner() throws OperatorCreationException {
    return new JcaContentSignerBuilder(signAlgorithmName).build(cryptoProvider.getPrivateKey());
  }
  ...
  private String sign(final byte[] data, X509Certificate cert)
    ...
    CMSSignedData singedData =
        signedDataGenerator.generate(new CMSProcessableByteArray(data), false);
