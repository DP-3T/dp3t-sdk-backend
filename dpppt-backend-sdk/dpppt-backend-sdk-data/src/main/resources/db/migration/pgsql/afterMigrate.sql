CREATE FUNCTION reassign_membership() RETURNS void AS $$
BEGIN
	IF EXISTS(SELECT current_setting('server_version_num')::int >= 110000) THEN
...
END;
$$ LANGUAGE plpgsql;
select reassign_membership();
DROP FUNCTION reassign_membership;
