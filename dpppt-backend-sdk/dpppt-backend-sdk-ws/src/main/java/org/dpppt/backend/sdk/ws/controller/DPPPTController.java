...
public class DPPPTController {
  ...
  // time in milliseconds that exposed keys are hidden before being served, in order to prevent
  // timing attacks
  private final long releaseBucketDuration;
  private final long requestTime;

  public DPPPTController(
  ...
  public @ResponseBody ResponseEntity<String> addExposee(
      @Valid
          ...
          @Documentation(
              description =
                  "The ExposeeRequest contains the SecretKey from the guessed infection date, the"
  ...
  @Documentation(
      ...
      responses = {
        "200=>Returns ExposedOverview in json format, which includes all exposed keys which were"
            + " published on _batchReleaseTime_",
  ...
  public @ResponseBody ResponseEntity<ExposedOverview> getExposedByDayDate(
      @PathVariable
          @Documentation(
              description =
                  "The batch release date of the exposed keys in milliseconds since Unix Epoch"
                      + " (1970-01-01), must be a multiple of 2 * 60 * 60 * 1000",
    ...
    List<Exposee> exposeeList =
        dataService.getSortedExposedForBatchReleaseTime(batchReleaseTime, releaseBucketDuration);
    ExposedOverview overview = new ExposedOverview(exposeeList);
    overview.setBatchReleaseTime(batchReleaseTime);
    return ResponseEntity.ok()
        .cacheControl(CacheControl.maxAge(Duration.ofMinutes(exposedListCacheControl)))
  ...
  @Documentation(
      description = "Query list of exposed keys from a specific batch release time",
      responses = {
        "200=>Returns ExposedOverview in protobuf format, which includes all exposed keys which"
            + " were published on _batchReleaseTime_",
        "404=>Couldn't find _batchReleaseTime_"
      })
    ...
    for (Exposee exposee : exposeeList) {
      Exposed.ProtoExposee protoExposee =
          Exposed.ProtoExposee.newBuilder()
              ...
              .setKeyDate(exposee.getKeyDate())
              .build();
      exposees.add(protoExposee);
    }
    Exposed.ProtoExposedList protoExposee =
  ...
  @Documentation(
      description = "Query number of available buckets in a given day, starting from midnight UTC",
      responses = {
        "200=>Returns BucketList in json format, indicating all available buckets since"
            + " _dayDateStr_"
      })
  public @ResponseBody ResponseEntity<BucketList> getListOfBuckets(
      @PathVariable
          @Documentation(
              description =
                  "The date starting when to return the available buckets, in ISO8601 date format",
              example = "2019-01-31")
          String dayDateStr) {
    ...
    while (currentBucket.getTimestamp()
        < Math.min(day.plusDays(1).getTimestamp(), now.getTimestamp())) {
