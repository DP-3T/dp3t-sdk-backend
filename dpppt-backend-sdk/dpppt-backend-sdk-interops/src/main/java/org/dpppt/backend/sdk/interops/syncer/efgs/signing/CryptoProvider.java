...
public class CryptoProvider {
  ...
  public CryptoProvider(String signClientCertificate, String signClientCertificatePassword)
    ...
    try (var byteInputStream = new ByteArrayInputStream(Base64.getDecoder().decode(signClientCertificate))) {
      ...
      this.privateKey = (PrivateKey)keyStore.getKey(this.alias, signClientCertificatePassword.toCharArray());
      this.certificate = (X509Certificate)keyStore.getCertificate(alias);
    }
  }

  /** Creates a CryptoProvider */
  public CryptoProvider(String signClientCertificate, String signClientCertificatePassword, String alias)
      throws CertificateException, IOException, KeyStoreException, NoSuchAlgorithmException, UnrecoverableKeyException {
    try (var byteInputStream = new ByteArrayInputStream(Base64.getDecoder().decode(signClientCertificate))) {
      this.keyStore = KeyStore.getInstance("PKCS12");
      this.keyStore.load(byteInputStream, signClientCertificatePassword.toCharArray());
      this.alias = alias;
      privateKey = (PrivateKey) keyStore.getKey(this.alias, signClientCertificatePassword.toCharArray());
      this.certificate = (X509Certificate) keyStore.getCertificate(alias);
    }
  }
